...
clc
% add utils folder
addpath(genpath('C:\Users\bhat_po\Desktop\RPS_github_walsh\utils\'))
...
for kk = 1:size(rps.seg.ROI,1)
    ...
    [rps.ri.A{1,kk}, ms] = generateWalshMeasurementMatrix_github(rps.seg.roim(1,kk),...
        rps.seg.roin(1,kk), rps.ri.numDecompLevels_Walsh, rps.seg.ROI{kk,2},...
        meas.remainingMeasurements);
...
for kk = 1:size(rps.seg.ROI,1)
    [V(kk),B,yB,ms] = findRefinementIndicatorsWalsh_github(rois_currentRes{1,kk},rps.seg.ROI{kk,1},...
        rps.ri.numDecompLevels_Walsh, rps.seg.ROI{kk,2} + 1, meas.remainingMeasurements,init.noiseDev);
